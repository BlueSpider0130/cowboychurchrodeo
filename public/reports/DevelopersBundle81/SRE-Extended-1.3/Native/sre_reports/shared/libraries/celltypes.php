...
function render($val, $type, $column, $is_conditional_formatting = true, $rating = true, $country = true) {
    global $images_path, $_CLEANED, $output_escaping, $cells;
        ...
        if ($show_real_size_image == false) {
            ...
            if ($result == false) {
                ...
                if ($show_realsize_in_popup)
                    $thum_code .= "<a href='" . $val . "' data-lightbox=image" . $uniqueKey . " data-title='Smart Report Engine' >";
...
function render_image_css($val) {
    ...
    $uniqueKey .= rand(1, 1000000);
    if ($show_realsize_in_popup)
        $original_image = "<a href='" . $val . "' data-lightbox=image" . $uniqueKey . " data-title='Smart Report Engine' >";
...
function arr_formatting_filter($arr, $col) {
    $array = array();
    foreach ($arr as $rule) {
        if (strtolower($rule ["column"]) == strtolower($col)) {
            $array [] = $rule;
        }
    }
    return $array;
