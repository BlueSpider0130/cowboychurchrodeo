...
class Search_options {
    ...
    public function __construct($type = "quick", $search_keyword, $dataType = "", $search_column = "", $second_search_keyWord = "") {
        ...
        $this->set_search_column($this->prepare_search_column($search_column));
    ...
    private function prepare_search_column($search_column) {
        if (strstr($search_column, "`")) {
            return $search_column;
        } elseif (strstr($search_column, ".")) {
            $arr = explode(".", $search_column);
            return "`" . $arr[0] . "`. `" . $arr[1] . "`";
        } else {
            return "`" . $search_column . "`";
        }
