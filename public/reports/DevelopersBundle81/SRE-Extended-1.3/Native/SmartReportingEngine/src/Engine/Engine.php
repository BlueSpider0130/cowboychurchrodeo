    ...
    protected function validate_request() {
        ...
        if (SRE__Auto__Replace__Reports__ == 0) {
            ...
            if (file_exists($this->_generated_report_directory_physical_path)) {
                ...
                Helper::log(\SRE\Engine\ErrorMessages::$messages[SRE__LANGUAGE__]["29"], 29);
                throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["29"], 29);
                return false;
            }
        }
        ...
        if ($this->_access_mode === SRE_PRIVATE_REPORT && count($this->_session_validation_login_keys) == 0) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["50"], 50);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["50"], 50);
            return false;
        }
        //case access mode is public yet there are session validations required
        if ($this->_access_mode === SRE_PUBLIC_REPORT && count($this->_session_validation_login_keys) != 0) {
        ...
        if ($this->_access_mode === SRE_PRIVATE_REPORT && $this->_login_page == "") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["52"], 52);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["52"], 52);
            return false;
        }
        //case private report and logout is empty
        if ($this->_access_mode === SRE_PRIVATE_REPORT && $this->_log_out_page == "") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["53"], 53);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["53"], 53);
            return false;
        }


        if ($this->connection_db_host == "" || $this->connection_db_name == "" || $this->connection_db_user == "") {
           Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["19"], 19);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["19"], 19);
            return false;
        }

        if ($this->_file_name == "" || $this->_file_name == "rep") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["20"], 20);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["20"], 20);
            return false;
        }

       if (SRE__REPORTS__DIR__ == "") {
           Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["21"], 21);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["21"], 21);
            return false;
        }

        if (SRE__Engin__DIR__ == "") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["22"], 22);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["22"], 22);
            return false;
        }

        if (SRE__CORE__DIR__ == "") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["23"], 23);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["23"], 23);
            return false;
        }


        if ($this->_datasource == "table" && empty($this->_table)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["16"], 16);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["16"], 16);
            return false;
        } elseif ($this->_datasource == "sql" && $this->_sql == "") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["17"], 17);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["17"], 17);
            return false;
        } elseif ($this->_datasource != "table" && $this->_datasource != "sql") {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["18"], 18);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["18"], 18);
            return false;
        }

        if (empty($this->_fields)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["15"], 15);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["15"], 15);
            return false;
        }
        ...
        if ($this->_datasource == "sql" && $this->_sql == "") {
            ...
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["25"], 25);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["25"], 25);
            return false;
        }

        if (!is_array($this->_group_by)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["26"], 26);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["26"], 26);
            return false;
        }

        if (!is_array($this->_sort_by)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["27"], 27);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["27"], 27);
            return false;
        }

        if (!is_array($this->_labels)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["28"], 28);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["28"], 28);
            return false;
        }

        if (!Helper::validate_filters_array($this->_tables_filters)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["35"], 35);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["35"], 35);
            return false;
        }

        if ($this->_datasource === "sql" && !empty($this->_tables_filters)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["36"], 36);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["36"], 36);
            return false;
        }
        ...
        if (count($this->_table) > 1 && !Helper::validate_table_exist($this->_labels, true)) {
           Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["37"], 37);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["37"], 37);
            return false;
        }
        if (count($this->_table) > 1 && !Helper::validate_table_exist($this->_cells, true)) {
           Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["38"], 38);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["38"], 38);
            return false;
        }
        if (count($this->_table) > 1 && count($this->_fields) > 1 && !Helper::validate_table_exist($this->_fields, false)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["39"], 39);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["39"], 39);
            return false;
        }
        if (count($this->_table) > 1 && !Helper::validate_table_exist($this->_group_by, false)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["40"], 40);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["40"], 40);
            return false;
        }
        if (count($this->_table) > 1 && !Helper::validate_table_exist($this->_sort_by, false)) {
           Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["41"], 41);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["41"], 41);

            return false;
        }

        if (count($this->_table) > 1 && empty($this->_relationships)) {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["42"], 42);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["42"], 42);
    ...
    protected function RecursiveMkdir($path) {
        if (!file_exists($path)) {
            ...
            if (mkdir($path, 0755)) {
                ...
                Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["1"], 1);
                throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["1"], 1);
            }
        }
    }
    ...
    protected function create_init_file() {
        ...
        if ($fp) {
            if (fwrite($fp, '<?php' . PHP_EOL)) {
                ...
                Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["2"], 2);
                throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["2"], 2);
            }
        } else {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["3"], 3);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["3"], 3);
        }
    }
    ...
    protected function create_report_config() {
        ...
        if ($fp) {
            if (fwrite($fp, '<?php' . PHP_EOL)) {
                ...
                Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["4"], 4);
                throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["4"], 4);
            }
        } else {
            Helper::log(ErrorMessages::$messages[SRE__LANGUAGE__]["5"], 5);
            throw new \Exception(ErrorMessages::$messages[SRE__LANGUAGE__]["5"], 5);
        }
    }
    ...
    public function create_report() {
        ...
        if ($this->validate_request()) {
            ...
            if (strtolower($this->_layout) != "mobile")
                ...
                return $this->_obj_report_path->get_report_url($this->_file_name, SRE_MOBILE_REDIRECT);
        } else {
            return false;
        }
    }
