...
class Filter {
    ...
    protected function filter($table, $column, $operator, $parameter, $parameter_type = SRE_NUMBER) {
        ...
        if (!empty($table) && !empty($column) && in_array(strtolower($operator), $allowed_operators) && in_array($parameter_type, $allowed_parameter_types)) {
            ...
            if ($parameter_type === SRE_DATE && Helper::is_date($parameter)) {
                $this_table_filter_array = array(
                    "sql" => "`" . $table . "`.`" . $column . "`  <->  " . $this->get_edited_operator(strtolower($operator)) . "  ?",
                    "param" => Helper::format_date_param($parameter, $parameter_type),
                    "type" => $this->get_edited_parameter_type($parameter_type)
                );
            } elseif ($parameter_type === SRE_NUMBER || $parameter_type === SRE_TEXT) {
    ...
    public function between($table, $column, $first_param, $second_param, $parameters_type = SRE_NUMBER) {
        $allowed_parameter_types = array(SRE_DATE, SRE_NUMBER, SRE_TEXT);
    ...
    public function more($table, $column, $param, $is_or_equal = false, $parameters_type = SRE_NUMBER) {
        $allowed_parameter_types = array(SRE_DATE, SRE_NUMBER, SRE_TEXT);
    ...
    public function less($table, $column, $param, $is_or_equal = false, $parameters_type = SRE_NUMBER) {
        $allowed_parameter_types = array(SRE_DATE, SRE_NUMBER, SRE_TEXT);
    ...
    public function equal($table, $column, $param, $parameters_type = SRE_NUMBER) {
        $allowed_parameter_types = array(SRE_DATE, SRE_NUMBER, SRE_TEXT);
    ...
    public function not_equal($table, $column, $param, $parameters_type = SRE_NUMBER) {
        $allowed_parameter_types = array(SRE_DATE, SRE_NUMBER, SRE_TEXT);
    ...
    public function like($table, $column, $param) {
        ...
        if (!empty($table) && !empty($column)) {
            $this->filter($table, $column, "like", $param, SRE_TEXT);
        }
    }
    ...
    public function not_like($table, $column, $param) {
        ...
        if (!empty($table) && !empty($column)) {
            $this->filter($table, $column, "not_like", $param, SRE_TEXT);
        }
    }
