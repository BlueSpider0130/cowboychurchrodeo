...
//put your code here
    ...
    public function __construct($access_mode, $login_page = Configs::SRE__DEFAULT_LOGIN_PAGE_, $logout_page = Configs::SRE__DEFAULT_LOGOUT_PAGE_,$session_name="") {
        $this->_access_mode = ($access_mode === Constants::SRE_PUBLIC_REPORT) ? Constants::SRE_PUBLIC_REPORT : Constants::SRE_PRIVATE_REPORT;
    ...
    private function session_add_validation_login_key($login_key, $check, $correct_data_type = Constants::SRE_TEXT, $correct_value_array = array()) {
        ...
        if (in_array($check, $checks) && !empty($login_key)) {
            ...
            switch ($check) {
                case "DATA_TYPE":
                    if (in_array($correct_data_type, json_decode(Constants::SRE_DATA_TYPES)))
    ...
    public function check_session_saved_user_key($session_user_key, $check_if_numeric = true) {
        ...
        if ($check_if_numeric)
            $this->session_add_validation_login_key($session_user_key, "DATA_TYPE", Constants::SRE_NUMBER);
    }
    ...
    public function check_session_saved_group_key($session_group_key, $allowed_group_array) {
            ...
            if (is_array($allowed_group_array) && count($allowed_group_array) > 1) {
                $this->session_add_validation_login_key($session_group_key, "IN_ARRAY",Constants::SRE_NUMBER , $allowed_group_array);
            } elseif (is_array($allowed_group_array) && count($allowed_group_array) === 1) {
                $this->session_add_validation_login_key($session_group_key, "EXACT_VALUE", Constants::SRE_NUMBER, $allowed_group_array);
            } elseif (!is_array($allowed_group_array) && !empty($allowed_group_array)) {
                $this->session_add_validation_login_key($session_group_key, "EXACT_VALUE", Constants::SRE_NUMBER, array($allowed_group_array));
            }
        }
    ...
    public function check_session_saved_exact_data_type_key($session_key, $correct_data_type) {
        ...
        if (in_array($correct_data_type, json_decode(Constants::SRE_DATA_TYPES)))
            ...
            Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["43"], 43);
            throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["43"], 43);
        }
    }
