...
class ReportOptions
{
    ...
    private $user = Configs::SRE__DEFAULT__USER__;
    private $pass = Configs::SRE__DEFAULT__PASS__;
    private $db = Configs::SRE__DEFAULT__DB__;
    private $host = Configs::SRE__DEFAULT__HOST__;
    private $language = Configs::SRE__DEFAULT_REPORT_LANGUAGE__;
    private $filter;
    private $formatter;
    private $access_mode;
     ...
     * @param string $access_mode  *The access mode you want of the generated report. The allowable access modes are : *
      Constants::SRE_PUBLIC_REPORT : For public reports with no session checks.
      Constants::SRE_PRIVATE_REPORT : For private reports*
     * @see get_file_name()
     * @see Constants::SRE_PUBLIC_REPORT
     * @code
     * //creating a public report based on a single table with the name "items"
      use  src\Engine as ReportLibrary;
      Require_once("../bootstrap.php");
      try {

      $report = new ReportLibrary\ReportOptions(Constants::SRE_PUBLIC_REPORT);
     ...
     *
     */
    public function __construct($access_mode =Constants::SRE_PRIVATE_REPORT , $data_source = Constants::SRE_Table, $report_name = "")
    {
        ...
        if ($data_source === Constants::SRE_SQL) {
            $this->datasource = "sql";
        } else {
            $this->datasource = "table";
        }

        if ($access_mode === Constants::SRE_PUBLIC_REPORT) {
            $this->access_mode = Constants::SRE_PUBLIC_REPORT;
        } else {
            $this->access_mode = Constants::SRE_PRIVATE_REPORT;
        }
    }
    ...
    public function set_MYSQL_Connection_userName($user = Configs::SRE__DEFAULT__USER__)
    {
        $this->user = $user;
        return $this;
    }
    ...
    public function set_MYSQL_Connection_password($password = Configs::SRE__DEFAULT__PASS__)
    {
        $this->pass = $password;
        return $this;
    }
    ...
    public function set_MYSQL_db_name($db = Configs::SRE__DEFAULT__DB__)
    {
        $this->db = $db;
        return $this;
    }
    ...
    public function set_MYSQL_hostname($host = Configs::SRE__DEFAULT__HOST__)
    {
        $this->host = $host;
        return $this;
    }
    ...
    {
        if (in_array(strtolower($language), Configs::SRE__ALLOWED_REPORT_LANGUAGES__)) {
            $this->language = strtolower($language);
        } else {
            $this->language = Configs::SRE__DEFAULT_REPORT_LANGUAGE__;
        }
        return $this;
    }
    ...
    {
        $trimmed_selected_tables = array_map("trim",$selected_tables);
        $this->table = $trimmed_selected_tables;
        return $this;
    }
    ...
    public function filter_between($table, $column, $first_param, $second_param, $parameters_type = Constants::SRE_NUMBER)
    {
        $this->filter->between($table, $column, $first_param, $second_param, $parameters_type);
        return $this;
    }

    public function filter_more($table, $column, $param, $is_or_equal = false, $parameters_type = Constants::SRE_NUMBER)
    {
        $this->filter->more($table, $column, $param, $is_or_equal, $parameters_type);
        return $this;
    }

    public function filter_less($table, $column, $param, $is_or_equal = false, $parameters_type = Constants::SRE_NUMBER)
    {
        $this->filter->less($table, $column, $param, $is_or_equal, $parameters_type);
        return $this;
    }

    public function filter_equal($table, $column, $param, $parameters_type = Constants::SRE_NUMBER)
    {
        $this->filter->equal($table, $column, $param, $parameters_type);
        return $this;
    }

    public function filter_not_equal($table, $column, $param, $parameters_type = Constants::SRE_NUMBER)
    {
    ...
    {
        $trimmed_group_by = array_map("trim",$group_by_array);
        $this->group_by = $trimmed_group_by;
        return $this;
    }
    ...
    public function security_init($login_page = Configs::SRE__DEFAULT_LOGIN_PAGE_, $logout_page = Configs::SRE__DEFAULT_LOGOUT_PAGE_, $session_name=Configs::SRE_DEFAULT_SESSION_NAME)
    {
        if ($this->access_mode == Constants::SRE_PUBLIC_REPORT) {
            Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["55"], 55);
            throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["55"], 55);
        } else {
    ...
    public function security_check_session_saved_user_key($session_user_key, $check_if_numeric = false)
    {
        if ($this->validate_key($session_user_key, __METHOD__) && $this->validate_access_mode()) {
            if (!is_null($this->report_security)) {
                ...
                Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 44);
                throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 44);
            }
        }
        return $this;
    }
    ...
    public function security_check_session_saved_group_key($session_group_key, $allowed_group_array)
    {
        if ($this->validate_key($session_group_key, __METHOD__)&& $this->validate_access_mode()) {
            if (!is_null($this->report_security)) {
                ...
                Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 45);
                throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 45);
            }
        }
        return $this;
    }
    ...
    public function security_check_session_saved_ip_key($session_ip_key)
    {
        if ($this->validate_key($session_ip_key, __METHOD__) && $this->validate_access_mode()) {
            if (!is_null($this->report_security)) {
                ...
                Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 46);
                throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 46);
            }
        }
        return $this;
    }
    ...
    public function security_check_session_saved_client_agent_key($session_client_agent_key)
    {
        if ($this->validate_key($session_client_agent_key, __METHOD__) && $this->validate_access_mode()) {
            if (!is_null($this->report_security)) {
                ...
                Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 49);
                throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 49);
            }
        }
        return $this;
    }
    ...
    public function security_check_session_saved_exact_value_key($session_key, $correct_value)
    {
        if ($this->validate_key($session_key, __METHOD__) && $this->validate_access_mode()) {
            if (!is_null($this->report_security)) {
                ...
                Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 47);
                throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 47);
            }
        }
        return $this;
    }
    ...
    public function security_check_session_saved_exact_data_type_key($session_key, $correct_data_type)
    {
        if ($this->validate_key($session_key, __METHOD__) && $this->validate_access_mode()) {
            if (!is_null($this->report_security)) {
                ...
                Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 48);
                throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["44"], 48);
            }
        }
        return $this;
    }
    ...
    private function validate_key($session_key, $method)
    {
        if (empty($session_key) || strstr($session_key, " ")) {
            Helper::log(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["54"], 54);
            throw new \Exception(ErrorMessages::$messages[Configs::SRE__LANGUAGE__]["54"] . "'  " . $method . " '", 54);
        } else {
            return true;
        }
    }
