...
$debug_clause = ($url_param === 1701) ? "&&DebugMode7=1701" : "";
...
if (!$empty_Report) {
    ...
    if (isset($_CLEANED ['start']) && isset($_CLEANED ["RequestToken"]) && $_CLEANED ["RequestToken"] == $_SESSION [$request_token]) {
        if (check_numeric_parameter($_CLEANED ['start'], $nRecords - 1)) {
            ...
            header("location: " . $_SERVER ["PHP_SELF"] . "?start=$_startRecord_index" . "&&RequestToken=$request_token_value" . $debug_clause);
    ...
    if (isset($_CLEANED ['cp']) && isset($_CLEANED ["RequestToken"]) && $_CLEANED ["RequestToken"] == $_SESSION [$request_token]) {
        if (check_numeric_parameter($_CLEANED ['cp'], $numberOfPages, false, 0)) {
            ...
            if (isset($_CLEANED ['start']) && $_CLEANED ['start'] != $_correct_startRecord_index) {
                ...
                header("location: " . $_SERVER ["PHP_SELF"] . "?start=$_correct_startRecord_index&&cp=$currentPage" . "&&RequestToken=$request_token_value" . $debug_clause);

            }
        } else {
            // case $cp is not clean

            header("location: " . $_SERVER ["PHP_SELF"] . "?cp=1" . "&&RequestToken=$request_token_value" . $debug_clause);
    ...
    $firstPage = $_SERVER ['PHP_SELF'] . "?cp=1" . "&&RequestToken=$request_token_value" . $debug_clause;
    $lastPage = $_SERVER ['PHP_SELF'] . "?cp=" . (int) $numberOfPages . "&&RequestToken=$request_token_value" . $debug_clause;
    $nextPage = $currentPage + 1 <= $numberOfPages ? $_SERVER ['PHP_SELF'] . "?cp=" . (int) ($currentPage + 1) . "&&RequestToken=$request_token_value" . $debug_clause : '#';
    $prevPage = $currentPage - 1 > 0 ? $_SERVER ['PHP_SELF'] . "?cp=" . (int) ($currentPage - 1) . "&&RequestToken=$request_token_value" . $debug_clause : '#';
...
elseif ($_export_option == 'pdf1' && version_compare(PHP_VERSION, '5.3.0') >= 0) {
    ...
    if (count($fields) > 5)
        get_pdf($sql, 'a4', 'landscape', 10, 10, 10, 10, 780, 800, 10, 11, true, $_startRecord_index, $records_per_page, $nRecords);
    else
        get_pdf($sql, 'a4', 'portrait', 10, 10, 10, 10, 490, 500, 9, 10, true, $_startRecord_index, $records_per_page, $nRecords);


}elseif ($_export_option == 'pdf1' && version_compare(PHP_VERSION, '5.3.0') == -1) {
    if (count($fields) > 5)
        get_pdf_large($sql, 'a4', 'landscape', 10, 10, 10, 10, 780, 800, 10, 11, true, $_startRecord_index, $records_per_page, $nRecords);
    else
        get_pdf_large($sql, 'a4', 'portrait', 10, 10, 10, 10, 490, 500, 9, 10, true, $_startRecord_index, $records_per_page, $nRecords);
    ...
    if (count($fields) > 8)
        get_pdf_large($sql, '', 'landscape', 10, 10, 10, 10, 780, 800, 10, 11, false, $_startRecord_index, $records_per_page, $nRecords);
    else
        get_pdf_large($sql, '', 'portrait', 10, 10, 10, 10, 490, 500, 9, 10, false, $_startRecord_index, $records_per_page, $nRecords);
...
$link_pdf_current = $_SERVER ["PHP_SELF"] . "?export=pdf1&&start=" . (int) $_startRecord_index . "&&RequestToken=$request_token_value" . $debug_clause;

$link_csv_current = $_SERVER ["PHP_SELF"] . "?export=csv1&&start=" . (int) $_startRecord_index . "&&RequestToken=$request_token_value" . $debug_clause;

$link_xml_current = $_SERVER ["PHP_SELF"] . "?export=xml1&&start=" . (int) $_startRecord_index . "&&RequestToken=$request_token_value" . $debug_clause;

$link_xls_current = $_SERVER ["PHP_SELF"] . "?export=xls1&&start=" . (int) $_startRecord_index . "&&RequestToken=$request_token_value" . $debug_clause;

$link_csv_all = $_SERVER ["PHP_SELF"] . "?export=csv" . "&&RequestToken=$request_token_value" . $debug_clause;

$link_xml_all = $_SERVER ["PHP_SELF"] . "?export=xml" . "&&RequestToken=$request_token_value" . $debug_clause;

$link_pdf_all = $_SERVER ["PHP_SELF"] . "?export=pdf" . "&&RequestToken=$request_token_value" . $debug_clause;

$link_pdf_xls = $_SERVER ["PHP_SELF"] . "?export=xls" . "&&RequestToken=$request_token_value" . $debug_clause;

// ********************************************/
// **************************print links******************************

$link_print1 = $_SERVER ["PHP_SELF"] . "?print=1&&start=" . (int) $_startRecord_index . "&&RequestToken=$request_token_value" . $debug_clause;

$link_print2 = $_SERVER ["PHP_SELF"] . "?print=2" . "&&RequestToken=$request_token_value" . $debug_clause;

$link_print_real = $_SERVER ['PHP_SELF'] . "?print=3&start=" . (int) $_startRecord_index . "&&RequestToken=$request_token_value" . $debug_clause;
...
if (!$empty_Report) {
    ...
    $link_next = $_SERVER ["PHP_SELF"] . "?start=$next_start" . "&&RequestToken=$request_token_value" . $debug_clause;
    ...
    $link_prev = $_SERVER ["PHP_SELF"] . "?start=$prev_start&&DebugMode7=1701" . "&&RequestToken=$request_token_value" . $debug_clause;
    ...
    if ($prev_record >= 0) {
        ...
        $prev_link = $_SERVER ['PHP_SELF'] . "?start=$prev_record&&RequestToken=$request_token_value" . $debug_clause;
    } else {
        $prev_link = '';
    }
}
