...
log_array($_POST);
log_array($_GET);
require_once ("codegniter/ci_input.php");
$captcha_files = array("captcha_image.php", "login.php", "enc.php", "forgot_password.php");
if (in_array(strtolower(basename($_SERVER ['PHP_SELF'])), $captcha_files)) {
...
$hc->charset = 'utf-8';
$hc->use_xss_clean = TRUE;
$hc->allow_CLEANED_array = TRUE;
...
function get($index, $data_type = "string", $global_array, $options = array(), $default = "") {
    ...
    if (isset($global_array [$index])) {
        $get = $hc->xss_clean($global_array [$index]);
    } else {
        return "";
    }
...
function send_log_info($maintainance_email) {
    ...
    if (filter_var($maintainance_email, FILTER_VALIDATE_EMAIL) && isset($_CLEANED ["DebugMode7"]) && $_CLEANED ["DebugMode7"] === 1701) {
